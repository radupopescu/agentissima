...
Ten defects that only a real model exposed, now fixed and covered by tests:
...
| **`spawn`'s container timeout was reported as `server_error`, not `timeout`.** `ContainerExecutor.run` wraps commands in `timeout --signal=TERM`, which exits `124` — the code `_exec` checks. `spawn`, the `pi` path, used `--signal=KILL`: the process dies of SIGKILL and `docker exec` reports `137` (128+9), so `timed_out` stayed `False`, `PiDriver` fell through to its no-`agent_end` branch, and every pi timeout was recorded as `server_error`. That corrupts §4.8's termination accounting and feeds §4.2's degenerate-rate detector the wrong category — one of the things the campaign exists to measure. Found live: five LFM-M8 Stage 2A runs at exactly `600.2s`, all labelled `server_error`, where `v4` pi had produced none. The parity suite covered `run` only, which is why it did not catch it | `spawn` now uses `--signal=TERM` too, keeping `124` the unambiguous timeout code and leaving `137` to mean what it should — killed by something else, an OOM against `--memory` most likely; conflating them would have hidden a real failure mode. `--kill-after` still guarantees death if pi ignores TERM. Two tests added: one pinning `spawn`'s contract, one asserting both paths agree, so they cannot drift apart again. No `task_set_version` bump — no pre-fix pi data survives, and it was a recording defect rather than a change to what is measured. The affected LFM-M8 pi stages were discarded and re-collected; `stage0`/`stage1` are `native`-only and were verified unaffected rather than assumed |
...
`run_task` prepares a fresh fixture copy, applies any variant, hashes the tree as its change
baseline, runs the driver, and grades. A new driver is a drop-in: implement the `Driver` signature and
